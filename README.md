...
- **반응형 디자인**: 페이지는 화면 크기에 따라 자동으로 조정됩니다.
- **차트와 정보 영역**: 차트와 주식 정보는 각각 깔끔하게 구분되어 있으며, 잘 정리된 카드 형식으로 제공됩니다.
- **버튼 디자인**: 버튼에는 호버 효과가 추가되어 시각적으로 피드백을 제공합니다.
- **애니메이션**: 차트를 접고 펼칠 수 있는 애니메이션 효과가 적용되어, 사용자 경험을 개선합니다.

#### 2.4. 푸터

- **푸터**: 페이지 하단에 저작권 정보가 포함된 푸터를 추가하여 사이트의 권리 정보를 제공합니다.

### 3. CSS 스타일

- **전체 스타일**: 페이지 배경색은 밝은 그레이(#f4f7f6)로 설정되어 있으며, 각 요소들은 단순하고 깔끔한 스타일을 제공합니다.
- **차트와 버튼**: 차트는 `max-width: 100%`로 설정되어 다양한 화면 크기에 대응하며, 버튼은 호버 시 색상이 변화하는 효과가 있습니다.
- **정보 카드**: 주식 정보를 담고 있는 카드들은 그림자 효과와 둥근 모서리로 스타일링 되어 있습니다.
